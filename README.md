...
<ul>
必要であれば、試験項目と実施エビデンスを含むドキュメントのご提供が可能です（Excel、日本語）。試験項目には xUnit を使った自動試験、半自動試験が含まれます。項目数は詳細が76で確認数は124となっています。<br>
プログラムによる結果確認が困難テストケースは人の目による確認が必要です。そういった項目についての試験結果はエビデンスシートに掲載しています。<br>
...
### 導入
最もシンプルな導入方法は、Visual Studio ソリューションに CFLog プロジェクトを加えることです。  

#### 手順 1.CFLog プロジェクトの追加
...
...
```
#### 手順 3.CFLog を使用するプロジェクトの global using に定義を追加
（CFLog プロジェクトへの参照の追加も必要です）
